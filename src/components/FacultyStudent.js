...
const FacultyStudent = ({ mobile, setMobile, faculty, admin, student }) => {
  return (
    <section className='w-full  h-screen bg-[#e5e5e5] flex flex-col'>
      ...
      <section className=" max-h-80] overflow-y-auto shadow-lg mt-4 p-2 grow">
        <ul className='w-full  flex flex-col gap-2'>
          {faculty
            .find((fac) => fac.faculty === admin.faculty)
            .students.map((element) => (
              <li key={element} className='w-full shadow-sm  p-4 font-Concert rounded-xl  flex  flex-col lg:flex-row justify-between  shadow-[#13213d] items-start lg:items-center gap-2 bg-[#fff] text-[#000]'>
                <span className="w-28 flex items-center gap-4">
                  ...
                  {student.find((std) => std._id === element).rollno}
                </span>
                <span className="w-40">
                  {student.find((std) => std._id === element).username}
                </span>
                <span className="w-52">
                  {student.find((std) => std._id === element).email}
                </span>
                <span className="w-52">
                  {student.find((std) => std._id === element).semester}
                </span>
              </li>
            ))}
        </ul>
